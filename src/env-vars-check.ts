...
if (APPLICATION.AUTO_ACTIVATE_NEW_USERS) {
  ;['REDIRECT_URL_ERROR', 'REDIRECT_URL_SUCCESS'].forEach((env) => {
...
if (STORAGE.ENABLED) {
  ;['S3_ENDPOINT', 'S3_BUCKET', 'S3_ACCESS_KEY_ID', 'S3_SECRET_ACCESS_KEY'].forEach((env) => {
    console.log(env, process.env[env], isUnset(process.env[env]))
    if (isUnset(process.env[env])) {
      errors.push(`Env var ${env} is required when storage is enabled but no value was provided`)
    }
...
if (PROVIDERS.apple) {
  ;['APPLE_CLIENT_ID', 'APPLE_TEAM_ID', 'APPLE_KEY_ID', 'APPLE_PRIVATE_KEY'].forEach((env) => {
    if (isUnset(process.env[env])) {
      errors.push(
