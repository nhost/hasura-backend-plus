...
-- TODO migration from Hasura-Backend-Plus v1 to v2

-- rename some tables, that we will copy from later
-- remove refresh table
DROP TABLE auth.refresh_tokens;
DROP TABLE auth.providers;

-- Create new tables for HBPv2
...
-- public.roles ---> auth.roles
INSERT INTO auth.roles (role) 
  SELECT role FROM public.roles;


--> public.user_roles ---> auth.account_roles
...
-- remove unused `users` columns, that has now been moved to `accounts`
ALTER TABLE users
  DROP COLUMN active, default_role, email, secret_token, secret_token_expires_at, register_data, is_anonymous;
